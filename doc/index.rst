...
We introduce the concept of **ownership** to deal with this.
Each parallel degree of freedom will have one owning process. 
Each rank will own a contiguous range of pdofs.

The pdofs to gdof mapping is stored in a :cpp:class:`pindex_map`

.. doxygenclass:: iceicle::pindex_map
   :members:

Note that gofs are ordered into three disjoint, contiguous subranges.
...
Ghost elements are elements on neighboring processes whose dofs will be necessary for operations on this process 
e.g face neighboring elements on interprocess faces.
