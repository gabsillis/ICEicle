    ...
    auto gen_vert() noexcept -> vertex_list<t>
{
  ...
  struct facet {
    ...
    template< std::size_t idim >
    [[nodiscard]] inline constexpr 
    auto get_facets_vert_indices(geo_code auto t)
    -> std::vector< facet<idim> >
    {
      if (idim == get_ndim(t)){
        ...
        auto base_facets = get_facets_vert_indices<idim - 1>(base_tcode(t));
        ...
        if(t[get_ndim(t) - 1] == prism_ext){
          auto top_facets = get_facets_vert_indices<idim - 1>(base_tcode(t));
