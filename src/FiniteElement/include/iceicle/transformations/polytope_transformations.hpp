  ...
  struct facet {
    ...
    -> std::vector< facet<idim> >
    {
      if (idim == get_ndim(t)){
        ...
        auto base_facets = get_facets<idim - 1>(base_tcode(t));
        ...
        if(t[get_ndim(t) - 1] == prism_ext){
          auto top_facets = get_facets<idim - 1>(base_tcode(t));
          for(facet<idim>& f : top_facets){
            for(std::size_t& idx : f.vertex_indices){
              idx += nvert(base_tcode(t));
            }
          }
